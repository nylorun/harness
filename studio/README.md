...
Studio lists registered agents and sessions, sends text, displays completed assistant responses and tool inputs/results, restores history, observes canonical SSE events, and cancels a turn. Each session shows chat beside an **Events** inspector (history `/items` + live SSE `/events`) and an optional Agent Manifest tab. **Model Settings** lists vault-stored providers, adds credentials in a right-side sheet, and switches the active provider/model. Responses appear when complete; token streaming, media, approvals UI, and remote deployment are deferred.
