import type { Json, RuntimeSessionState, WireSessionEvent } from "./contracts.js";
...
export function renderHeader(
  ...
  if (info.resolved !== undefined) {
    const capabilities = info.resolved.capabilities.portableHistory ? "portable-history" : "no-portable-history";
    lines.push(style.dim(`connection ${info.resolved.route} · ${info.resolved.accessMode} · ${info.resolved.protocol} · ${capabilities}`));
  }
  if (info.resolved?.credential !== undefined) {
    // The variable and where it was found, never the value.
    lines.push(
      style.dim(
        `credential ${info.resolved.credential.variable} from ${info.resolved.credential.source}`
      )
    );
  }
 ...
 */
export function renderEvent(event: WireSessionEvent, style: Style): Rendered | undefined {
  const payload = event.payload;
  switch (event.type) {
    case "final": {
      const delta = text(payload, "output");
...
export function renderFooter(state: RuntimeSessionState, style: Style): string {
  const cost = state.costUsd > 0 ? ` · $${state.costUsd.toFixed(4)}` : "";
  return style.dim(
    `${state.status} · ${state.turns} turns · ${state.tokensIn}in/${state.tokensOut}out${cost}`
  );
}
