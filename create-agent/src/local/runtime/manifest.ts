...
export function describeTools(entries: readonly { file: string; exports: Record<string, unknown> }[]): ToolDescriptor[] {
  const tools = entries.map(({ file, exports }) => {
    ...
    if (schema.type !== "object") {
      throw new NyloBuildError(diagnostic(
        "NYLO_BUILD_TOOL_SCHEMA_UNSUPPORTED",
        "build",
        "error",
        file,
        "A tool input schema must be a Zod object schema.",
        "Use z.object({...}) as the tool input schema."
      ));
    }
