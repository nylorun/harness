  ...
  return {
    ...
    async writeBundle() {
      ...
      if (!loaded.config || !Array.isArray(loaded.config.secrets) || !Array.isArray(loaded.config.mcp) || typeof loaded.config.harness !== "function") {
        throw new NyloBuildError(diagnostic("NYLO_BUILD_DEFINITION_INVALID", "build", "error", "agent/agent.ts", "The default export is not a Harness-backed Run(...) definition.", "Default-export Run((model, directive) => Agent(model, directive), {...})."));
      }
