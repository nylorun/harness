...
export async function runRecords(args: Args): Promise<number> {
  ...
  const recorder = createFileRecorder({ projectRoot: resolve(args.project) });
  ...
  if (subcommand === "show") {
    ...
    const summary = await recorder.summary(id);
    const events = await recorder.read(id);
    if (summary === undefined || events === undefined) {
      process.stderr.write(`${style.red("error")} no record for session ${id}.\n`);
      return 1;
    }
    if (args.json) {
      process.stdout.write(envelope("runs show", true, { summary, events }));
      return 0;
    }
  ...
  if (subcommand === "delete") {
    ...
    const removed = await recorder.remove(id);
    if (args.json) {
