...
Runtime publishes root-relative discovery and endpoint URLs. `basePath` must match the Hono mount path so advertised manifests and AG-UI endpoints resolve. Agent-scoped routes inside the router are already under `/agents/:id/...`, so mounting at `/agents` yields live paths like `/agents/agents/:id/v1/ag-ui`. Pass the same prefix for other mounts:
