...
/** Protocol assertions against real Harness agents. */
    ...
    it("discovers neutral identity, records ordered lifecycle events, and preserves conversation history", async () => {
      ...
      try {
        ...
        for (const content of ["first", "second"]) {
          const result = await app.request("http://local/echo/v1/ag-ui", {
            method: "POST",
        ...
        const events = await (
          await app.request("http://local/echo/v1/sessions/conversation/events")
        ).json();
        ...
        const history = await (
          await app.request("http://local/echo/v1/ag-ui/sessions/conversation")
        ).json();
        expect(history.messages).toHaveLength(4);
        expect(
          (await app.request("http://local/missing/manifest.json")).status,
        ).toBe(404);
      } finally {
        await runtime.close();
      }
    ...
    it.each(["approval", "response"] as const)(
      ...
      async (kind) => {
        ...
        try {
          ...
          const session = await (
            await app.request("http://local/echo/v1/sessions/waiting")
          ).json();
