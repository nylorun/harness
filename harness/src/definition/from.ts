...
export function agentFrom<Info = unknown>(
  ...
  for (const capability of manifest.capabilities) {
      ...
      (async (request, next) => {
        if (tools.length)
          request.configuration.tools.set(
            capability.id,
            tools.map((tool) => implementations[capability.id]?.tools?.[tool.name] ?? tool),
          );
    ...
    dynamics.set(capability.id, {
      ...(impl.beforeModelCall ? { beforeModelCall: impl.beforeModelCall as any } : {}),
      ...(impl.afterModelCall ? { afterModelCall: impl.afterModelCall as any } : {}),
...
function normalizeManifest(json: AgentManifest | JsonObject): AgentManifest {
  ...
  return deepFreeze({
    ...
    ...(value.outputSchema && typeof value.outputSchema === "object"
      ...
      : {}),
    capabilities: (value.capabilities as CapabilityManifest[]).map(normalizeCapability),
  });
}

function normalizeCapability(capability: CapabilityManifest): CapabilityManifest {
  return Object.freeze({
    ...
    ...(capability.instructions
      ...
      : {}),
    ...(capability.tools ? { tools: Object.freeze(capability.tools.map(normalizeTool)) } : {}),
    ...(capability.beforeModelCall ? { beforeModelCall: true } : {}),
    ...(capability.afterModelCall ? { afterModelCall: true } : {}),
  });
}
