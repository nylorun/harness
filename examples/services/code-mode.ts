...
import { z } from "zod";
import type { JsonValue, ToolDefinition, ToolExecutionContext } from "@nylorun/harness";
...
export async function runCodeProgram(
  ...
  let value: unknown;
  try {
    value = await withBudget(Promise.resolve(pending), context.signal, timeoutMs);
  } catch (error) {
...
export function renderToolsSdk(roster: readonly ToolDefinition[]): string {
  const bindings = [...roster]
    ...
    .map((tool) => {
      const comment = tool.description ? `  /** ${escapeJsdoc(tool.description)} */\n` : "";
...
function createBindings(
  ...
  const bindings = Object.create(null) as Record<string, (args: unknown) => Promise<unknown>>;
  for (const tool of roster) {
    bindings[tool.name] = async (args: unknown) => {
      ...
      if (outcome.kind === "completed") {
        calls.push({ name: tool.name, args: recordedArgs, output: outcome.output });
        return outcome.output;
      }
      if (outcome.kind === "failed") {
        calls.push({ name: tool.name, args: recordedArgs, error: outcome.message });
        throw new ToolCallError(tool.name, outcome.message);
      }
      if (outcome.kind === "denied") {
        calls.push({ name: tool.name, args: recordedArgs, error: outcome.reason });
  ...
  return new Proxy(Object.freeze(bindings), {
    get(target, property) {
      if (typeof property === "string" && Object.hasOwn(target, property)) return target[property];
...
function argsType(tool: ToolDefinition): string {
  try {
    return jsonSchemaToTs(z.toJSONSchema(tool.parameters, { target: "draft-07" }));
  } catch {
...
function jsonSchemaToTs(schema: unknown): string {
  ...
  if (Array.isArray(node.enum) && node.enum.every(isJsonLiteral)) {
    return node.enum.map((value) => JSON.stringify(value)).join(" | ") || "unknown";
  }
  if (Array.isArray(node.anyOf) || Array.isArray(node.oneOf)) {
    const variants = (Array.isArray(node.anyOf) ? node.anyOf : node.oneOf) as unknown[];
  ...
  if (node.type === "array") return `readonly (${jsonSchemaToTs(node.items)})[]`;
...
async function withBudget<T>(promise: Promise<T>, signal: AbortSignal, timeoutMs: number): Promise<T> {
  ...
  const abort = new Promise<never>((_, reject) => {
    ...
    timer = setTimeout(() => reject(new Error("run_code timed out.")), timeoutMs);
  });
  try {
