...
export const SKILLS_CATALOG = projectAsset("agent/skills/catalog");
...
async function loadRoster(source: SkillsSource): Promise<SkillRoster> {
  ...
  if ("directories" in source)
    return SkillRoster.fromDirectories(source.directories);
...
function readSkillResourceTool(roster: SkillRoster, listed: readonly Skill[]) {
  return tool({
    ...
    description:
      "Read a supporting file from a skill directory after load_skill.",
      ...
      try {
        return {
          ...
          output: {
            name,
            path,
            content: await roster.readResource(name, path),
          },
        };
      } catch (error) {
