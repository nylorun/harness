  ...
  for (const event of events) {
    if (event.type === "tool.sealed") {
      const calls = Array.isArray(
        (event.payload.attributes as Record<string, unknown> | undefined)
          ?.executable,
      )
        ? ((event.payload.attributes as Record<string, unknown>)
            .executable as readonly Record<string, unknown>[])
        : [];
      for (const call of calls) {
        const id = typeof call.callId === "string" ? call.callId : undefined;
        const name =
          typeof call.toolName === "string" ? call.toolName : undefined;
        if (!id || !name) continue;
        output.push(
