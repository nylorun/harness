...
The **Interior Design** agent uses a separate OpenAI Images credential, so it works even when the
chat model uses OpenRouter or another OpenAI-compatible endpoint. Set `OPENAI_API_KEY` and
optionally `OPENAI_IMAGE_MODEL` (default: `gpt-image-2`). Uploaded room images are materialized as
inline data only for the configured vision-capable `NYLO_MODEL`; Harness events keep opaque asset
references. Original and generated bytes live only under `.data/media/` until you remove `.data`.
