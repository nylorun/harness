...
export class SkillRoster {
  ...
  async readResource(name: string, path: string): Promise<string> {
    ...
    if (!skill)
      throw new SkillResourceError("unknown_skill", `Unknown skill '${name}'.`);
    ...
    if (skill.files && normalized in skill.files)
...
type ParsedSkill =
  | {
      ...
      readonly warnings: readonly SkillDiagnostic[];
    }
...
function parseSkillMarkdown(
  ...
  if (!fields)
    return { ok: false, message: "SKILL.md frontmatter could not be parsed." };
  const name = typeof fields.name === "string" ? fields.name.trim() : "";
  const description =
    typeof fields.description === "string" ? fields.description.trim() : "";
  if (!description)
    return { ok: false, message: "SKILL.md is missing a description." };
  if (description.length > 1024) {
    return {
      ok: false,
      message: "SKILL.md description exceeds 1024 characters.",
    };
  }
