  ...
  return {
    ...
    tools: [
        ...
        async execute(_, context) {
          try {
            return {
              ...
              output: { files: await workspace(((context.scope as { sessionId?: string } | undefined)?.sessionId ?? context.executionId)).list() },
            };
          } catch (error) {
        ...
        async execute({ path }, context) {
          try {
            return {
              ...
              output: {
                path,
                content: await workspace(((context.scope as { sessionId?: string } | undefined)?.sessionId ?? context.executionId)).read(path),
              },
            };
          } catch (error) {
            return failure(error);
      ...
      tool({
        name: "write_file",
        description:
          "Write a UTF-8 file to the isolated workspace after approval.",
        inputSchema: z.object({
        ...
        async execute({ path, content }, context) {
          if (context.resume?.approved === false) {
            return {
              ...
              reason: "The user declined the workspace write.",
            };
          }
          try {
            await workspace(((context.scope as { sessionId?: string } | undefined)?.sessionId ?? context.executionId)).write(path, content);
            return {
              kind: "completed" as const,
      ...
      tool({
        ...
        async execute({ script }, context) {
          try {
            return {
              kind: "completed" as const,
              output: await workspace(((context.scope as { sessionId?: string } | undefined)?.sessionId ?? context.executionId)).shell(
                script,
                context.signal,
              ),
            };
...
function failure(error: unknown) {
  return {
    ...
    message:
      error instanceof Error ? error.message : "Docker sandbox unavailable.",
  };
}
